jQuery.noConflict()
var ddimagepanner={
	magnifyicons: ['../images/maps/zoom_in.png','../images/maps/zoom_out.png', 30,27], //set path to zoom in/out images, plus their dimensions
	...
	init:function($, $img, options){
		...
		if (s.canzoom=="yes"){ //enable image zooming?
			...
			s.$statusdiv=$('<div> 1x </div>').appendTo(s.$pancontainer) //create DIV to show current magnify level
	...
	zoomfunct:function($, $img, s){
		...
		var $zoomimages=$('<img src="'+magnifyicons[0]+'" /><img src="'+magnifyicons[1]+'" />')
			.css({width:magnifyicons[2], height:magnifyicons[3], cursor:'pointer', zIndex:1000, position:'absolute',
						top:s.wrappersize[1]-magnifyicons[3]-1, left:s.wrappersize[0]-magnifyicons[2]-3, opacity:0.3
			})
			.attr("title", "Zoom Out")
			.appendTo(s.$pancontainer)
			$zoomimages.eq(0).css({left:parseInt($zoomimages.eq(0).css('left'))-magnifyicons[2]-3, opacity:1}) 
			.attr("title", "Zoom In")
			...
			if (newzoom==1) 
				$zoomimages.eq(1).css("opacity", 0.3)
			else if (newzoom==ddimagepanner.maxzoom) 
				$zoomimages.eq(0).css("opacity", 0.3)
